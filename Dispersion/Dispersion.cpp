...
void Read(unsigned int** input_array)
{
	/*std::string line;
	...
	int i = 0;
	while (fscanf(f, "%d %d %d %d %d %d %d %d %d %d", &input_array[i][0], &input_array[i][1], &input_array[i][2], &input_array[i][3], &input_array[i][4], &input_array[i][5], &input_array[i][6], &input_array[i][7], &input_array[i][8], &input_array[i][9]) != EOF) {
		i++;
	}
	fclose(f);
	}
